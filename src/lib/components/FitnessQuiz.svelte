<script lang="ts">
	...
	// Ids of previously answered questions, used only to number the
	// progress indicator ("Вопрос N") since the tree branches and the
	// numeric id itself isn't a reliable step count.
...
<div class="mx-auto flex w-full max-w-xl flex-col items-center px-6 py-16 sm:py-20">
	<div class="relative min-h-[460px] w-full" aria-live="polite">
		{#key currentId}
			<div in:fade={{ duration: questionFadeIn }} out:fade={{ duration: questionFadeOut }} class="absolute inset-0 flex w-full flex-col items-center text-center">
				{#if currentNode.type === 'choice'}
					<p class="mb-6 text-sm font-medium tracking-wide text-gray-400">
						Вопрос {history.length + 1}
					</p>
				...
				{:else}
					<p class="mb-6 text-sm font-medium tracking-wide text-gray-400">Ваша программа</p>

					<h2 class="text-xl leading-snug font-light text-gray-900 sm:text-2xl">
